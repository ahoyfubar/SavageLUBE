...
function filterComments(appInfo) {
    ...
    if (document.body.classList.contains('article-section-savage-love')) {
        ...
        for (var i=0; i<comments.length; i++) {
            ...
            var user = appInfo.users.find(u => u.name.toLowerCase() === name.toLowerCase());
...
function addAvatarSelector(comment, name) {
    ...
    image[0].onclick = function() {
        ...
        if (result) {
            var img = new Image();
            img.onload = function() {
                safari.extension.dispatchMessage("changeAvatar",  { "name": name, "action": "avatar", "avatar": result });
...
*/
    ...
    button.onclick = function(){
        if (button.classList.contains('unmute-user')) {
            safari.extension.dispatchMessage("blockUser",  { "name": name, "action": "unmute" });
        }
        else if (confirm("Are you sure you want to mute " + name + "?")) {
            safari.extension.dispatchMessage("blockUser",  { "name": name, "action": "mute" });
        }
    };
